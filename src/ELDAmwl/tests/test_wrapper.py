...
def scipy_func(a, axis=None, **kwargs):
    return axis
...
class TestWrapper(unittest.TestCase):
    ...
    def test_bad_tupel(self):
        with self.assertRaises(SciPyWrapperAxisError):
             x = scipy_reduce_wrapper(scipy_func)(5, axis=())
        with self.assertRaises(SciPyWrapperAxisError):
             x = scipy_reduce_wrapper(scipy_func)(5, axis=(1, 2))
    ...
    def test_other_type(self):
        with self.assertRaises(SciPyWrapperAxisError):
             x = scipy_reduce_wrapper(scipy_func)(5)
        with self.assertRaises(SciPyWrapperAxisError):
             x = scipy_reduce_wrapper(scipy_func)(5, axis='time')
