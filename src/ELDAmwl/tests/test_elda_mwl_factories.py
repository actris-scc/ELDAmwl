...
def test_MeasurementParams(mocker):
    # REALLY important! Patch where the function is imported not where the function is defined!!!!!!
    mocker.patch('ELDAmwl.elda_mwl_factories.read_system_id', return_value=0)
    mocker.patch('ELDAmwl.elda_mwl_factories.read_mwl_product_id', return_value=1)

    val = AttrDict({'Products': AttrDict({'ID': 1})})

    mocker.patch('ELDAmwl.elda_mwl_factories.get_products_query', return_value=[val])
